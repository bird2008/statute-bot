...
module.exports = {
  ...
  run(msg, args) {
    ...
    let hembed = new MessageEmbed()
      .setColor(0xf72121)
      .setTitle("❌ | Bot nie ma permisji do wykonania tej komendy!")
      .setFooter(`KOLEGA - ${member.username}`)
      .setTimestamp();
    ...
    if (!guild.me.permissionsIn(channel).has(["MANAGE_MESSAGES"])) {
      return msg.channel.send({ embeds: [hembed] });
    }

    let aembed = new MessageEmbed()
      .setColor(0xf72121)
      .setTitle("❌ | Podaj ilość wiadomości do usunięcia!")
      .setFooter(`KOLEGA - ${memberr.username}`)
      .setTimestamp();

    let bembed = new MessageEmbed()
      .setColor(0xf72121)
      .setTitle("❌ | Nie możesz usunąć więcej niż 100 wiadomości!")
      .setFooter(`KOLEGA - ${memberr.username}`)
      .setTimestamp();

    if (!args[0] || args[0] < 1) return msg.channel.send({ embeds: [aembed] });

    if (args[0] < 100 && !args[0] < 1) {
      let embed = new MessageEmbed()
        .setColor(0x4bf542)
        .setTitle("✅ | Usunąłeś " + args[0] + " wiadomości!")
        .setFooter(`KOLEGA - ${memberr.username}`)
        .setTimestamp();
    ...
    if (args[0] > 100) return msg.channel.send({ embeds: [bembed] });
